// 0x7672F0 @ 0x7672F0
// MoonSugarEffect decode: non-skinned geometry shader/draw path reached from RenderTriGeometries. Receives NiGeometry, geomData, NiSkinInstance/null, NiTransform*, bound pointer, and BuffData; native path owns shader/pass setup, constants, stream/index binding, draw, and cleanup.
    ...
    if ( defaultShader->__vftable->Unk48((NiD3DShaderInterface *)defaultShader) ) /*0x767376*/
    {
      do /*0x7674ce*/
      {
        ...
        v10 = (NiGeometryBufferData *)defaultShader->__vftable->Unk3C( /*0x7673ed*/
                                        ...
                                        0,
                                        (UInt32)v10,
                                        (UInt32)this->member.super.propertyState);// MoonSugarEffect decode: pre-SetupShaderPrograms hook window. At this point ebx=active NiD3DShader, edi=NiDX9Renderer, ebp=NiGeometry, eax/soon esi=bound BuffData. Strong Parallax predicate: *(ebx+0x34)==0 and *(ebx+0x3C)==*(NiD3DPass**)0x00B47620 before swapping CurrentPass+0x58.
        defaultShader->__vftable->SetupShaderPrograms( /*0x7673fb*/
          ...
          (int)this->member.super.dynamicEffectState,
          a5,
          a6);                                  // MoonSugarEffect target gap: non-skinned draw loop has just bound pass state/streams and now calls SetupShaderPrograms. A per-vertex shader swap must be scoped before/around this call, then constants are live before DrawPrimitive/DrawIndexedPrimitive.
        ...
        if ( v10->IB )                          // MoonSugarEffect decode: post-SetupShaderPrograms/post-constant-flush hook window. Replacement VS is already bound in D3D state; restore CurrentPass+0x58 to the original wrapper here, then upload raw Moon Sugar c26/c27 constants before DrawIndexedPrimitive/DrawPrimitive. /*0x767412*/
        {
          v16 = 0; /*0x76741a*/
          v15 = 0; /*0x76741e*/
