// 0x00767520 @ 0x00767520
// MoonSugarEffect decode: skinned geometry shader/draw path. Chooses skin-instance shader property shader or renderer defaultShader, runs shader interface methods over hardware/software skin partitions, flushes constants, draws indexed partitions, then cleanup.
              ...
              else
              {
                ...
                ((void (__thiscall *)(NiObject *, int, _DWORD *, int, _DWORD *, _DWORD, _DWORD, int, int, int, int, _DWORD *))v15->__vftable->Unk_0E)( /*0x7676e4*/
                  ...
                  *(_DWORD *)(a1 + 0x10),
                  ...
                  v25,
                  a11,
                  a12);                         // MoonSugarEffect decode: skinned per-partition SetupShaderPrograms call (shader vtable +0x38). At this point ebx=NiD3DShaderInterface, esi=NiDX9Renderer, ebp=skin partition, edi=returned partition NiGeometryBufferData/BuffData from +0x3C. This is the skinned analogue of 0x7673FB for a scoped replacement vertex shader.
                (*(void (__thiscall **)(_DWORD))(**(_DWORD **)(*(_DWORD *)(a1 + 0x8AC) + 0xFF0) + 4))(*(_DWORD *)(*(_DWORD *)(a1 + 0x8AC) + 0xFF0));// MoonSugarEffect decode: shader constant-manager flush after skinned SetupShaderPrograms and before DrawIndexedPrimitive. A raw MoonSugar vertex constant upload belongs after this flush and before the draw, with stock bone constants preserved. /*0x7676f7*/
                v29 = 0; /*0x7676fe*/
                v30 = 0; /*0x767702*/
                if ( v16[0x11] )                // MoonSugarEffect decode: skinned post-SetupShaderPrograms/post-flush hook window. Replacement vertex shader is already bound in D3D state here; restore pass wrapper before draw if a scoped skin replacement is ever enabled, preserving bone constants. /*0x7676fb*/
                {
                  do /*0x76776e*/
                  {
