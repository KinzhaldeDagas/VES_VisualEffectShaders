...
| Image-space render path | Implemented for the byte-validated `NiRendererRender_ImageSpaceProcessCall` path. Profile-family `.pso` files are the normal shipped path. Build 89 requires current standalone profile shaders for HeadWound, Blind, Dim, On Fire, HeatShimmer, Rain, Frost, and DepthMaskDebug so those profiles cannot silently render stale combined fallback semantics. | Menu, render-to-texture, reset, and unusual render-mode validation. |
...
Profile fallback correction note: Build 89 extends the fail-closed profile shader rule to On Fire, Rain, Frost, and DepthMaskDebug. The older combined `VESDistortion.pso` remains a compatibility dispatcher, but it is not accepted as current evidence for those profile-family effects.

Dim variant fidelity note: Build 82 routes `VESDimWhiteInverted`, `VESDimPurple`, and `VESDimDarkRed` through the existing Dim profile-family shader using the profile variant constant. Dim profiles now require the current standalone `VESDimPS.pso`; the combined fallback is not treated as the current implementation path for those variants. Build 87 preserves that model by storing the three color variants as `VESDim100` active renderer profile executions with variant constants `2.0`, `3.0`, and `4.0`; the requested command name is retained only for console/log identity. Build 88 removes the Dim variant radial/luma-heavy presentation so the variants remain uniform Dim color treatments rather than Blind-like peripheral occlusion.
